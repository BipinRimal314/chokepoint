...
The detail behind the [README](../README.md): the policy language, every
check chokepoint makes and why, the measurements behind the design, the
evidence log, telemetry, deployment, and known limitations. For a
plain-language account start with [how-it-works.md](how-it-works.md).
...
threshold sits between those. [docs/single-tool-sweep.md](single-tool-sweep.md)
...
and [docs/single-tool-sweep.md](single-tool-sweep.md) records why, along
...
Applications](owasp-asi-mapping.md) states which of the ten risks
...
`v0.3.0` is released, with signed binaries for Linux, macOS and Windows.
It is tested end to end against the reference filesystem and fetch servers
with Claude Code and with a scripted hijacked agent: see
[real-agent-test.md](real-agent-test.md). It has also run on a k3s cluster,
written up in [first-cluster-run.md](first-cluster-run.md). It has not yet
been run by outside users against their own agents, which is the next
step.

Two larger pieces of work are open:
...
- **Streamable-HTTP transport**, which unblocks four controls that have nothing
  ...
  and read the note there about session identity before starting.
- **Making a single-tool sweep scoreable at all.** This is a research problem,
  not a port: it needs a signal that survives a constant tool sequence. See
  [Known defect](#known-defect-a-single-tool-sweep-is-invisible).

The smaller items, and what is verified rather than only tested, are kept in
order in [project-state.md](project-state.md).
